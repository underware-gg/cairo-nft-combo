...
use oz_token::systems::cash::{
    cash, ICashDispatcher, ICashDispatcherTrait,
};
use oz_token::models::coin_config::{CoinConfig};
...
fn setup_uninitialized(faucet_amount: u256) -> (IWorldDispatcher, ICashDispatcher) {
    ...
    world.grant_owner(dojo::utils::bytearray_hash(@"oz_token"), coin.contract_address);
    // world.grant_writer(selector_from_tag!("oz_token-CoinConfig"), coin.contract_address);
    let call_data: Span<felt252> = array![
        OWNER().into(),
        faucet_amount.low.into(),
        faucet_amount.high.into(),
    ].span();
    world.init_contract(selector_from_tag!("oz_token-cash"), call_data);
